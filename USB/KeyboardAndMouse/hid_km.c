...
// Keyboard Variables
volatile uint8_t KB_Packet_Buf[8] = {0x00};
volatile uint8_t HID_KB_LED_Status = 0x00;
// Mouse Variables
volatile uint8_t MS_Packet_Buf[4] = {0x00};
...
const uint8_t KeyboardReportDesc[62] = {
...
const uint8_t MouseReportDesc[52] = {
    0x05, 0x01,  // Usage Page (Generic Desktop)
...
// Keyboard Functions
uint8_t HID_KB_UploadPacket (uint32_t wait) {
    while (wait && HID_KM_GetEndpointBusy (HID_KB_EP)) { ; }
    return READY == (HID_KM_EpUpload (HID_KB_EP, KB_Packet_Buf, sizeof (KB_Packet_Buf)));
}

uint32_t HID_KB_SetKeyDown (HID_KBKeyCode key) {
    uint8_t i;
    for (i = 2; i < 8; i++) {
...
void HID_KB_KeyDown (HID_KBKeyCode key) {
    if (HID_KB_SetKeyDown (key))
        HID_KB_UploadPacket (1U);
}

uint32_t HID_KB_SetKeyUp (HID_KBKeyCode key) {
    uint8_t i;
    for (i = 2; i < 8; i++) {
        // look for used slot
        if (KB_Packet_Buf[i] == (uint8_t)key) {
...
void HID_KB_SetModKeyDown (uint8_t modkey) {
    KB_Packet_Buf[0] |= (modkey);
}

void HID_KB_ModKeyDown (uint8_t modkey) {
    HID_KB_SetModKeyDown (modkey);
    HID_KB_UploadPacket (1U);
}

void HID_KB_SetModKeyUp (uint8_t modkey) {
    KB_Packet_Buf[0] &= (~(modkey));
}

void HID_KB_ModKeyUp (uint8_t modkey) {
    HID_KB_SetModKeyUp (modkey);
    HID_KB_UploadPacket (1U);
}
...
void HID_KB_TypeString (uint8_t *str, uint16_t len) {
    for (uint16_t i = 0; i < len; i++) {
        ...
        if (str[i] >= 0x31 && str[i] <= 0x39) {  // num1-9
            keycode =
                (HID_KBKeyCode)(HID_KBKeyCode_1_Excalm + (str[i] - 0x31));
        } else if (str[i] == 0x30) {                    // num0
            keycode = HID_KBKeyCode_0_RRndBracket;
        } else if (str[i] >= 0x61 && str[i] <= 0x7a) {  // lowerCase letter
            modkey =
                HID_KB_CheckLEDStatus (HID_KBLed_Caps) ? HID_KBModKey_LShift : 0x00;
            keycode = (HID_KBKeyCode)(HID_KBKeyCode_A + (str[i] - 0x61));
        } else if (str[i] >= 0x41 && str[i] <= 0x5a) {  // upperCase letter
            modkey =
                HID_KB_CheckLEDStatus (HID_KBLed_Caps) ? 0x00 : HID_KBModKey_LShift;
...
// Mouse Functions
static uint8_t HID_MS_UploadPacket (uint32_t wait) {
    while (wait && HID_KM_GetEndpointBusy (HID_MS_EP)) { ; }
    return READY == (HID_KM_EpUpload (HID_MS_EP, MS_Packet_Buf, sizeof (MS_Packet_Buf)));
}
